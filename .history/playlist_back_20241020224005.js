...
// Route to scrape and update a playlist
// 7. Route to handle scrape and update operation
app.post('/scrape-and-update', async (req, res) => {
    try {
        ...
        const {
            name,
            description,
            playlists, // Playlists are expected to be in JSON string format from the formData.
            sortBy,
            maxAgeMonths,
            trackLimit,
            token,
        ...
        // Filter tracks by release date if maxAgeMonths is specified
        if (minReleaseDate) {
        ...
        // Remove duplicate tracks by their URI
        ...
        // Limit the number of tracks according to the specified track limit
        const limitedTracks = tracks.slice(0, trackLimit || 50);
        ...
        // Update the playlist with the selected tracks

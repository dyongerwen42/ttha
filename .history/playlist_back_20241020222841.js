...
app.post('/scrape-and-update', async (req, res) => {
    const { name, description, playlists, sortBy, maxAgeMonths, trackLimit, token, playlistId } = req.body;
    console.log('Scrape and update operation:', { name, playlistId });
    ...
    try {
        ...
        for (const playlist of playlists) {
            console.log('Scraping playlist:', playlist);
            const playlistTracks = await getTracksFromPlaylist(playlist, token);
            tracks = tracks.concat(playlistTracks);
        }
        ...
        const limitedTracks = tracks.slice(0, trackLimit || 50);
        console.log('Total tracks after scraping and filtering:', limitedTracks.length);

        // Create or update the playlist with the scraped tracks
        const finalPlaylistId = playlistId || await findOrCreatePlaylist(name, description, token);
        await updatePlaylistWithTracks(finalPlaylistId, limitedTracks, token);
        console.log('Playlist updated:', { name, playlistId: finalPlaylistId });

        res.json({ name, playlistId: finalPlaylistId, tracks: limitedTracks });
    } catch (error) {
        console.error('Error in scrape-and-update operation:', error);
        res.status(500).send('Error in scrape-and-update operation');
    }
});

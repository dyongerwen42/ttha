...
app.post('/create-or-update-playlist', async (req, res) => {
    const { name, description, playlistId, token } = req.body;
    let { tracks } = req.body;
    tracks = tracks || [];
    ...
    try {
        ...
        if (req.files && req.files.image) {
            ...
            await uploadPlaylistImage(finalPlaylistId, base64Image, token);
        }
...
app.get('/user-playlists', async (req, res) => {
    ...
    try {
        ...
        const playlistsData = await fetchWebApi(`v1/users/${userId}/playlists?limit=50`, 'GET', null, token);
        res.json(playlistsData.items);
    } catch (error) {
...
async function uploadPlaylistImage(playlistId, base64Image, token) {
    const endpoint = `v1/playlists/${playlistId}/images`;

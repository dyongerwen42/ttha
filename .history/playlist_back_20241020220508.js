...
// 4. Route to scrape and add tracks to the specified playlist
// 4. Route to scrape and add tracks to the specified playlist
app.post('/scrape-and-update', async (req, res) => {
    const { name, description, playlists, sortBy, maxAgeMonths, trackLimit, token } = req.body;
    console.log('Scraping and updating tracks for playlist:', { name, description, playlists });
    ...
    try {
        ...
        if (!Array.isArray(playlists) || playlists.length === 0) {
            console.error('No playlists provided');
            return res.status(400).send('At least one playlist is required');
        }

        // Fetch tracks based on the selected playlists
        const tracks = await scrapeTracksFromPlaylists(playlists, sortBy, maxAgeMonths, trackLimit, token);
        console.log(`Total tracks fetched after scraping: ${tracks.length}`);

        // Create or update the playlist
        const finalPlaylistId = await findOrCreatePlaylist(name, description, token);
        console.log(`Using playlist ID: ${finalPlaylistId}`);
        await updatePlaylistWithTracks(finalPlaylistId, tracks, token);
        
        res.json({ name, playlistId: finalPlaylistId });
    } catch (error) {
        console.error('Error in scrape and update:', error);
        res.status(500).send('Error processing scrape and update');
    }
});

// Helper function to scrape tracks from selected playlists
async function scrapeTracksFromPlaylists(playlists, sortBy, maxAgeMonths, trackLimit, token) {
    let allTracks = [];
    const currentDate = new Date();
    const minReleaseDate = maxAgeMonths ? new Date(currentDate.setMonth(currentDate.getMonth() - maxAgeMonths)) : null;
    console.log('Minimum release date for filtering:', minReleaseDate);
    ...
    for (const playlistId of playlists) {
        ...
        // Filter tracks by release date if applicable
        if (minReleaseDate) {
            const filteredTracks = playlistTracks.filter(track => new Date(track.releaseDate) >= minReleaseDate);
            console.log(`Filtered down to ${filteredTracks.length} tracks from playlist: ${playlistId}`);
    ...
    // Remove duplicates and limit tracks if necessary
    console.log('Removing duplicate tracks...');
    allTracks = removeDuplicates(allTracks);
    console.log(`Total unique tracks after filtering duplicates: ${allTracks.length}`);

    // Sort tracks based on specified criteria
    if (sortBy === 'popularity') {
        console.log('Sorting tracks by popularity');
    ...
    // Limit the number of tracks to the specified track limit
    const limitedTracks = allTracks.slice(0, trackLimit || 50);
    console.log(`Returning ${limitedTracks.length} tracks to the client.`);

...
mongoose
  ...
  .catch((err) => {
    if (err.name === "MongoNetworkError") {
      console.error("Network error while connecting to MongoDB:", err);
    } else if (err.name === "MongoTimeoutError") {
      console.error("MongoDB connection timed out:", err);
    } else if (err.name === "MongoParseError") {
      console.error("Error parsing MongoDB connection string:", err);
    } else {
      console.error("Unexpected MongoDB connection error:", err);
    }
    process.exit(1); // Stop the application if it cannot connect to the database
  });
...
app.get("/blogs", async (req, res) => {
  try {
    // Fetch all published blogs from the database
    const blogs = await Blog.find({ visibility: /published/i }).sort({ date: -1 });

...
// Connect to MongoDB
mongoose.connect('mongodb+srv://thavastgoedonderhoudwebsite:421142Dcdc@cluster0.069us.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0', {
...
const generateRSSFeed = async () => {
  const feed = new RSS({
    ...
    custom_elements: [
      ...
      { 'atom:link': { _attr: { href: rssBaseInfo.feed_url, rel: 'self', type: 'application/rss+xml' } } }  // Self-reference to match correct feed location
    ]
  });
  ...
  blogs.forEach((blog) => {
    const blogUrl = `http://localhost:3000/blogs/${encodeURIComponent(blog.title.replace(/\s+/g, '-'))}`;
    const featuredImageUrl = `http://localhost:3000/${encodeURIComponent(blog.featuredImage)}`;  // Encode the image URL
    ...
    feed.item({
      ...
      custom_elements: [ // Content wrapped in CDATA
        { 'media:content': {
          _attr: {
...
app.post('/create-blog', upload.single('featured_image'), async (req, res) => {
  try {
    const {
      title, description, mediatype, content, date, categories,
      visibility, seo_title, seo_description, seo_keywords,
    } = req.body;
        ...
        if (req.body.dynamic_fields) {
            try {
                ...
                if (!Array.isArray(dynamicFields)) {
                    console.warn('Dynamic fields provided is not an array, ignoring dynamic fields.');
                    dynamicFields = [];
                }
            } catch (parseError) {
                console.error('Error parsing dynamic fields:', parseError);
                return res.status(400).send({ message: 'Invalid dynamic fields format. Please provide valid JSON.', error: parseError.message });
            }
        }

        // Prepare dynamic fields text for email
        const dynamicFieldsText = dynamicFields.length > 0 ? dynamicFields.map(field => `${field.question}: ${field.answer}`).join('\n') : 'No dynamic fields provided';
...
${dynamicFieldsText}`,
        ...
        transporter.sendMail(mailOptions, (error, info) => {
            if (error) {
                ...
                return res.status(500).send({ message: 'Failed to send email. Please try again later.', error: error.message });
            }
        ...
        res.status(500).send({ message: 'An unexpected error occurred while submitting the form. Please try again later.', error: error.message });
    }
});
  ...
  Bestand: ${req.file ? req.file.path : 'Geen bestand geüpload'}`,
      ...
      transporter.sendMail(mailOptions, (error, info) => {
        if (error) {
          ...
          return res.status(500).send({ message: 'Verzenden van e-mail is mislukt. Probeer het later opnieuw.', error: error.message });
        }
        console.log('E-mail succesvol verzonden:', info.response);
        res.status(200).send({ message: 'Demo-aanvraag succesvol verzonden en e-mail is verstuurd.' });
      });
    } catch (error) {
      console.error('Fout bij het verwerken van de demo-aanvraag:', error);
      res.status(500).send({ message: 'Er is een onverwachte fout opgetreden bij het verwerken van uw aanvraag. Probeer het later opnieuw.', error: error.message });
    }
  });  
...
app.get('/binnenschilderwerk', async (req, res) => {
    try {
        ...
        console.error(err);
        res.status(500).send('An error occurred while retrieving Binnenschilderwerk blogs.');
    }
});
...
app.get('/buitenschilderwerk', async (req, res) => {
    try {
        ...
        console.error(err);
        res.status(500).send('An error occurred while retrieving Buitenschilderwerk blogs.');
    }
});
...
app.get('/report-ct', (req, res) => {
    ...
    res.set('Expect-CT', 'max-age=86400, enforce, report-uri="https://tha-diensten.nl/report-ct"');
...
app.get('/timmerwerk', async (req, res) => {
    try {
        ...
        console.error(err);
        res.status(500).send('An error occurred while retrieving Timmerwerk blogs.');
    }
});
...
app.get('/blogs/:title', async (req, res) => {
    ...
    try {
      ...
      const blog = await Blog.findOne({ title: new RegExp(`^${blogTitle}$`, 'i') });
  
      if (!blog) {
        return res.status(404).send('Blog not found');
      }
  
      const recentPosts = await Blog.find({ visibility: 'published' }).sort({ date: -1 }).limit(3);
